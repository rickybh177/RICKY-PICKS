...
const { PLANS, isSubscription, ALL_MODELS } = require('./plans');
...
function productsOfPlan(planId) {
  ...
  if (p && p.choose) return [];                 // "a elegir": los productos son del usuario
  if (p && Array.isArray(p.products)) return p.products;
  return [String(planId).split('_')[0]];
}

/* boughtProducts: la elección de la compra cuando lo comprado es "a
   elegir" (tres_mensual) — sin ella un tres no cubriría nada.
   Un mensual "a elegir" viejo solo lo cubre lo que trae TODOS los
   modelos (todo_mensual) o el mismo plan (re-suscripción). */
function coveredBy(boughtPlan, monthlyPlan, boughtProducts) {
  if (monthlyPlan === boughtPlan) return true;
  const bought = Array.isArray(boughtProducts) && boughtProducts.length ? boughtProducts : productsOfPlan(boughtPlan);
  const m = PLANS[monthlyPlan];
  if (m && m.choose) return ALL_MODELS.every(prod => bought.includes(prod));
  return productsOfPlan(monthlyPlan).every(prod => bought.includes(prod));
}

/* `keep` (opcional): { stripeSubId, preapprovalId, products } — la
   suscripción RECIÉN creada cuando lo comprado también es mensual, y
   la elección de modelos si lo comprado es "a elegir". Sin esto, subir
   de un mensual individual al combo mensual cancelaría el combo mismo
   (un plan siempre se cubre a sí mismo) y el cliente pagaría el alta
   de algo que se acaba de cancelar. */
async function cancelCoveredRecurring(userId, userEmail, boughtPlan, keep) {
  keep = keep || {};
  const summary = { stripe: 0, mercadopago: 0 };
  ...
  try {
    if (process.env.STRIPE_SECRET_KEY && userEmail) {
      ...
      for (const c of customers.data) {
        ...
        for (const s of subs.data) {
          const meta = s.metadata || {};
          if (s.id === keep.stripeSubId) continue; // la recién creada no se toca
          if (meta.user_id === userId && meta.plan && isSubscription(meta.plan) && coveredBy(boughtPlan, meta.plan, keep.products)) {
  ...
  try {
    const token = process.env.MP_ACCESS_TOKEN;
    if (token) {
      const cubiertos = Object.keys(PLANS).filter(p => isSubscription(p) && coveredBy(boughtPlan, p, keep.products));
      for (const planId of cubiertos) {
        ...
        for (const p of (data.results || [])) {
          ...
          if (p.external_reference !== `${userId}:${planId}`) continue;
          if (p.id === keep.preapprovalId) continue; // la recién creada no se toca
          if (p.status === 'authorized' || p.status === 'pending') {
            const upd = await fetch(`https://api.mercadopago.com/preapproval/${p.id}`, {
              method: 'PUT',
              headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
module.exports = { cancelOtherRecurring, cancelCoveredRecurring, coveredBy };
