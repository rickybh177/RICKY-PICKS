...
/* MERCADOS QUE NUNCA SE REGALAN (regla del dueño, 24-sep-2026).

   La doble oportunidad —"León o empate (1X)"— es el pick más aburrido
   que produce el modelo: se lee como "casi no puedes perder" y no
   genera ninguna emoción, así que no sirve para promocionar. Vale para
   TODAS las ligas, no solo Liga MX.

   Ojo con el alcance: esto bloquea el ESCAPARATE, no el modelo. Quien
   paga sigue viendo la doble oportunidad en la página de su liga, que
   es donde ese mercado sí sirve. Aquí solo se decide qué se enseña
   gratis.

   El empate seco ("Empate", del 1X2) SÍ se puede regalar: ese sí tiene
   emoción. Lo que se prohíbe es el "gana X o empate". */
const MERCADOS_NO_REGALABLES = ['double_chance'];

function esRegalable(v) {
  if (!v || !v.label) return false;
  if (MERCADOS_NO_REGALABLES.includes(v.market)) return false;
  /* Respaldo por si algún modelo nuevo no manda `market`: la etiqueta
     de la doble oportunidad siempre trae "o empate" y su (1X) / (X2). */
  const t = normaliza(v.label);
  return !(t.includes('o empate') || /\((\s*)(1x|x2)(\s*)\)/.test(t));
}
...
/* El veredicto que la card le enseña al visitante: manda el BET (y
   ...
   exactamente lo que el cliente va a ver. Los mercados no regalables
   quedan fuera ANTES de elegir: así no solo no se enseñan, tampoco
   inflan el puntaje de un partido que sin ellos no daría. */
const RANGO = { bet: 3, maybe: 2, skip: 1 };
function veredictoMostrado(verdicts) {
  const list = (verdicts || []).filter(esRegalable);
...
function veredictoForzado(verdicts, mercado) {
  ...
  if (!m) return null;
  /* La regla de los no regalables vale TAMBIÉN aquí: el dueño dijo
     "nunca, en ningún pick gratis". Si un override pidiera una doble
     oportunidad, no se enseña y el llamador cae al veredicto normal. */
  return (verdicts || []).find(v => esRegalable(v) && normaliza(v.label).includes(m)) || null;
}
...
module.exports = { elegir, evalua, puntaje, atractivo, cuantosGrandes, esRegalable, MERCADOS_NO_REGALABLES, veredictoMostrado, veredictoForzado, veredictoDeCard, normaliza, BANDA, TAQUILLEROS };
