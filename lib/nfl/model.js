...
const { manualFor, availableMarkets } = require('./manual-odds');
...
async function buildWeek(weekParam, seasontypeParam) {
  ...
  const [games, oddsMap, manualMap] = await Promise.all([
    getWeek(SEASON, week, seasontype),
    getOddsApiLines(),
    getManualMap('nfl'), // momios capturados (Playdoit) — mandan sobre todo
  ]);
  ...
  for (const g of games) {
    ...
    /* momios del mercado: almacén compartido primero, archivo local
       de respaldo (misma precedencia dentro de marketFor) */
    const manualEntry = lookupManual(manualMap, g.date, g.away.abbr, g.home.abbr) || manualFor(g);
    const market = marketFor(g, oddsMap, { home: g.home.full, away: g.away.full }, manualEntry);
    ...
    /* Con momios capturados a mano, la casa manda: solo se proponen
       los mercados que ESA casa abre en ESE juego. Proponer un pick
       que el usuario no puede colocar (típico: 1ª mitad en
       pretemporada) es ruido que además ensucia el récord. */
    const avail = availableMarkets(manualEntry);
    const shown = avail ? verdicts.filter(v => avail.has(v.market)) : verdicts;

    const strength = Math.max(0, ...shown.map(v => v.verdict === 'bet' ? v.prob : 0));
    ...
    out.push({
      ...
      verdicts: shown,
      /* mercados que la casa abre en este juego (null = sin momios
         manuales, se muestran todos los del modelo) */
      available_markets: avail ? [...avail] : null,
      markets: {
        ...mk,
        ...
        market_odds: market ? {
          ...
          manual: market.manual || undefined,
          spread: market.spread,
          total: market.total,
          ml_home: (market.raw && market.raw.ml_home) != null ? market.raw.ml_home : probToAm(market.ml_home_imp),
          ml_away: (market.raw && market.raw.ml_away) != null ? market.raw.ml_away : probToAm(market.ml_away_imp),
          spread_home: market.raw && market.raw.spread_home,
          spread_away: market.raw && market.raw.spread_away,
          total_over: market.raw && market.raw.total_over,
          total_under: market.raw && market.raw.total_under,
        } : null,
        fair_odds: {
  ...
  return {
    ...
    /* cuántos juegos usan momios capturados a mano (la casa real):
       el chip de arriba debe decir la verdad de dónde salen */
    manual_odds: (() => {
      const n = out.filter(g => !g.error && g.markets && g.markets.market_odds && g.markets.market_odds.manual);
      if (!n.length) return null;
      return { games: n.length, source: n[0].markets.market_odds.source };
    })(),
    games: out,
  };
}
