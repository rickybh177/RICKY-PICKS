...
async function buildWeek(weekParam, seasontypeParam) {
  ...
  /* vista automática (sin semana pedida): si la semana en curso ya
     se jugó COMPLETA, avanzar a la siguiente — con rollover de
     pretemporada a temporada regular. ESPN deja su "semana actual"
     clavada en la jugada hasta mitad de semana, y la página debe
     estar siempre en lo que viene, no en lo que ya pasó. */
  if (!Number(weekParam)) {
    let guard = 0;
    while (guard++ < 25 && games.length && games.every(g => g.state === 'post')) {
