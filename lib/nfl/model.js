...
const { simulateGame, marketsFromSims, N_SIMS } = require('./engine');
...
/* pretemporada: más alto que en regular (el mercado sabe qué coach
   juega titulares), pero no 0.5 — con 0.5 el edge se muere aunque el
   modelo vea claro (probado con los momios reales de Playdoit del
   Salón de la Fama 2026: ARI +1.5 al 58.9% del modelo salía SKIP).
   Los ratings de pretemporada ya están validados fuera de muestra,
   así que 0.35 es el punto medio defendible. */
const PRE_BLEND = 0.35;
...
/* ---- etiquetas de los picks ----
   Cada pick se nombra con el MERCADO tal como aparece en la casa
   de apuestas (Playdoit: "Ganador", "Hándicap", "Totales"), para
   poder encontrarlo sin traducir. "Cardinals ganan por 1 o menos"
   no decía de qué mercado era y se confundía con el moneyline.
   La explicación en español llana baja al subtexto. */
function spreadLabel(teamName, line) {
  if (line == null) return `Hándicap: ${teamName}`;
  return `Hándicap: ${teamName} ${line > 0 ? '+' : ''}${line}`;
}
/* Qué tiene que pasar para COBRAR (lectura conservadora: en las
   líneas enteras el margen exacto es devolución, no premio). */
function spreadGloss(line) {
  if (line == null) return null;
  if (line === 0) return 'gana el juego';
  if (line < 0) return `gana por ${Math.floor(Math.abs(line)) + 1} o más`;
  const margen = Math.ceil(line) - 1;
  return margen > 0 ? `gana, o pierde por ${margen} o menos` : 'gana el juego';
}
...
async function buildWeek(weekParam, seasontypeParam) {
  ...
  let seasontype = Number(seasontypeParam) === 1 ? 1 : Number(seasontypeParam) === 2 ? 2 : phase.seasontype;
  let week = Math.min(seasontype === 1 ? PRE_WEEKS : REG_WEEKS, Math.max(1, Number(weekParam) ||
    (seasontype === phase.seasontype ? phase.week : 1)));

  let games = await getWeek(SEASON, week, seasontype);

  /* vista automática (sin semana pedida): si en la semana en curso ya
     no queda NADA POR EMPEZAR, avanzar a la siguiente — con rollover
     de pretemporada a temporada regular. ESPN deja su "semana actual"
     clavada en la jugada hasta mitad de semana, y la página debe
     estar siempre en lo que viene, no en lo que ya pasó.

     Antes se exigía que TODOS estuvieran `post`, así que el lunes por
     la noche —con 15 juegos terminados y solo el de la noche en
     curso— la semana siguiente no aparecía hasta el martes. Un juego
     que ya arrancó tampoco es "lo que viene": su línea ya no se puede
     tomar. Quien quiera seguirlo vuelve con la flecha ‹ del selector
     de semana, que sigue ahí. */
  if (!Number(weekParam)) {
    let guard = 0;
    while (guard++ < 25 && games.length && !games.some(g => g.state === 'pre')) {
      if (seasontype === 1 && week >= PRE_WEEKS) { seasontype = 2; week = 1; }
      else if (seasontype === 2 && week >= REG_WEEKS) break;
      else week++;
      games = await getWeek(SEASON, week, seasontype);
    }
  }

  const preseason = seasontype === 1;
  const maxWeek = preseason ? PRE_WEEKS : REG_WEEKS;
  const oddsMap = await getOddsApiLines(seasontype); // FUENTE ÚNICA de momios
  ...
  for (const g of games) {
    ...
    /* momios del mercado (The Odds API, única fuente) */
    ...
    // Spread — EV real contra el precio (si hay línea del mercado)
    {
      ...
      const gloss = spreadGloss(line);
      verdicts.push({
        market: 'spread',
        label: spreadLabel(team.name, line),
        prob: p != null ? +p.toFixed(3) : null,
        edge: ev, verdict,
        line_txt: (gloss ? `${team.abbr} ${gloss}` : team.abbr) +
    ...
    // Total
    {
      ...
      verdicts.push({
        market: 'total',
        label: `Total: ${side === 'over' ? 'Más' : 'Menos'} de ${mk.total.line} puntos`,
        prob: p != null ? +p.toFixed(3) : null,
        edge: ev, verdict,
        line_txt: `entre los dos equipos · modelo espera ${mk.total.model_total}` +
          (hasLine ? '' : ' · línea del modelo, aún sin momios'),
      });
    }
    // Moneyline
    {
      ...
      verdicts.push({
        market: 'moneyline',
        label: `Ganador: ${team.name}`,
        prob: +p.toFixed(3),
        edge: ev, verdict,
        line_txt: `${team.abbr} gana el juego` +
          (priceImp != null ? ` · ${probToAm(priceImp) > 0 ? '+' : ''}${probToAm(priceImp)}` : ' · aún sin momios'),
      });
    }
    // 1ª mitad (sin momios publicados: barras, tope MAYBE)
    {
      ...
      verdicts.push({
        market: 'h1_total',
        label: `Total 1ª mitad: ${side === 'over' ? 'Más' : 'Menos'} de ${mk.first_half.total_line} puntos`,
        prob: p != null ? +p.toFixed(3) : null,
        edge: null,
        verdict: preseason
          ? barVerdict(p, PRE_BARS_NO_ODDS.h1[0], PRE_BARS_NO_ODDS.h1[1])
          : barVerdict(p, 1.01, 0.575),
        line_txt: `solo los 2 primeros cuartos · modelo espera ${mk.first_half.model_total}`,
      });
    }

    /* Los props de jugador viven en /api/nfl-props (bajo demanda, con
       historial real por jugador y momios por evento): aquí solo se
       deja el ENTORNO del juego que ese motor necesita. Sin props en
       pretemporada: el titular juega un cuarto. */
    const env = preseason ? null : {
      exp_home: +expHome.toFixed(1), exp_away: +expAway.toFixed(1),
      odds_event_id: (market && market.odds_event_id) || null,
    };
    ...
    out.push({
      ...
      markets: {
        ...
        market_odds: market ? {
          ...
          spread_home: probToAm(market.spread_home_imp),
          spread_away: probToAm(market.spread_away_imp),
          total_over: probToAm(market.total_over_imp),
          total_under: probToAm(market.total_under_imp),
        } : null,
        fair_odds: {
  ...
  return {
    ...
    mu_shift: muShift,
    odds_source: oddsMap ? 'the-odds-api' : null, // única fuente; null = sin key o sin cobertura
    games: out,
  };
}
