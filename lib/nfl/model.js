...
/* Umbrales por fase. En pretemporada la señal es MUCHO más débil
   (Brier apenas 0.2457 vs 0.2500 de "todos iguales" en la prueba
   contra 2025), así que solo pasan las discrepancias grandes:
   el doble de EV que en temporada regular. Y sin momios
   publicados no hay veredicto — no existe precio contra el cual
   tener ventaja, y una "barra de probabilidad" en agosto sería
   inventarse confianza que el backtest no respalda. */
const BARS = {
  regular: { spread: { bet: 0.03, maybe: 0.01 }, total: { bet: 0.025, maybe: 0.01, floor: 0.3 }, ml: { bet: 0.03, maybe: 0.01 } },
  pre:     { spread: { bet: 0.06, maybe: 0.03 }, total: { bet: 0.06,  maybe: 0.03, floor: 0.3 }, ml: { bet: 0.06, maybe: 0.03 } },
};
...
function buildAnalysis(g, mk, market, wxNotes, restNotes, preseason) {
  ...
  const parts = [];
  if (preseason) {
    parts.push('Pretemporada: aquí el modelo NO usa los ratings de la temporada — están medidos y no predicen agosto, porque juegan los suplentes. Usa ratings sacados solo de pretemporadas anteriores (qué tan en serio se toma agosto cada organización), que sí persisten pero apenas: es señal débil.');
  }
...
async function buildWeek(weekParam, seasontypeParam) {
  ...
  for (const g of games) {
    ...
    // Spread — EV real contra el precio (si hay línea del mercado)
    {
      ...
      if (market && market.spread != null && dv) {
        ...
        // en pretemporada, sin precio no hay veredicto
        verdict = preseason ? 'skip' : barVerdict(p, 1.01, 0.57);
      }
    ...
    // Moneyline
    {
      ...
      if (priceImp != null && dv) {
        ...
        verdict = preseason ? 'skip' : barVerdict(p, 1.01, 0.60); // sin precio: tope MAYBE
      }
      verdicts.push({
        market: 'moneyline',
    ...
    // 1ª mitad (sin momios publicados: barras, tope MAYBE)
    {
      ...
      verdicts.push({
        ...
        prob: p != null ? +p.toFixed(3) : null,
        edge: null,
        verdict: preseason ? 'skip' : barVerdict(p, 1.01, 0.575),
  ...
  /* honestidad sobre la pretemporada: medido en 2021-25, agosto es
     casi impredecible (los ratings de temporada no correlacionan,
     la localía es ~0, el desempeño de pretemporada previa apenas
     corr 0.21). El modelo da su lectura pero avisa que el producto
     de verdad arranca en la semana 1. Sin esto el tablero se ve
     "roto" cuando todo sale SKIP — y lo correcto ES que salga SKIP. */
  const withOdds = out.filter(g => !g.error && g.market_source).length;
  const disclosure = preseason ? {
    level: 'baja',
    headline: 'La pretemporada casi no se puede predecir — y el modelo lo dice de frente.',
    detail: 'Medido sobre 245 juegos de agosto (2021-2025): los ratings de la temporada regular NO predicen la pretemporada (correlación ≈ 0, juegan los suplentes), la ventaja de local se cae a 0.1 puntos, y lo único que persiste es qué tan en serio se toma agosto cada organización — señal real pero débil. Por eso vas a ver muchos SKIP: el modelo no encuentra ventaja contra el mercado y prefiere decírtelo a inventarte un pick. El tablero de verdad arranca en la Semana 1.',
    games_with_odds: withOdds,
    games_total: out.filter(g => !g.error).length,
  } : null;
