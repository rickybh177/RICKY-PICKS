...
function buildAnalysis(g, mk, market, wxNotes, restNotes, preseason) {
  ...
  const parts = [];
  if (preseason) {
    parts.push(`Pretemporada: los ratings de la temporada no pesan aquí (en agosto mandan los suplentes y lo que cada coach quiera probar), así que el modelo juega con el entorno real de pretemporada — ${PRESEASON.mu} puntos por equipo y localía de ${PRESEASON.hfa} — y se apoya más en el mercado.`);
  }
...
async function buildWeek(weekParam, seasontypeParam) {
  ...
  if (preseason) {
    R = {};
    for (const ab in PRIORS) R[ab] = { off: PRIORS[ab].off * PRESEASON.beta, def: PRIORS[ab].def * PRESEASON.beta };
    mu = PRESEASON.mu; hfa = PRESEASON.hfa;
    /* el entorno anotador de agosto deriva año con año (2025 anotó
       +6 pts/juego vs lo que decía 2024): aprender de los juegos
       de ESTA pretemporada ya terminados, con encogimiento fuerte */
    try {
      ...
      if (n > 0) {
        muShift = Math.max(-2.5, Math.min(2.5, (obs / (2 * n) - PRESEASON.mu) * (n / (n + 16))));
  ...
  for (const g of games) {
    ...
    // Spread — EV real contra el precio (si hay línea del mercado)
    {
      ...
      if (market && market.spread != null && dv) {
        const priceImp = side === 'home'
          ...
          : (market.spread_away_imp != null ? market.spread_away_imp : IMP_110);
        const v = evVerdict(p, priceImp, side === 'home' ? dv.spread[0] : dv.spread[1], { blend });
        verdict = v.verdict; ev = v.ev;
      } else {
        verdict = barVerdict(p, 1.01, 0.57); // sin línea real: tope en MAYBE
      }
      verdicts.push({
        market: 'spread',
    ...
    // Total
    {
      ...
      if (market && market.total != null && dv) {
        const priceImp = side === 'over'
          ...
          : (market.total_under_imp != null ? market.total_under_imp : IMP_110);
        const v = evVerdict(p, priceImp, side === 'over' ? dv.total[0] : dv.total[1], { bet: 0.025, maybe: 0.01, floor: 0.3, blend });
        verdict = v.verdict; ev = v.ev;
      } else {
        verdict = barVerdict(p, 1.01, 0.57);
      }
      verdicts.push({
        market: 'total',
    ...
    // Moneyline
    {
      ...
      if (priceImp != null && dv) {
        const v = evVerdict(p, priceImp, side === 'home' ? dv.ml[0] : dv.ml[1], { blend });
        verdict = v.verdict; ev = v.ev;
      } else {
        verdict = barVerdict(p, 1.01, 0.60); // sin precio: tope en MAYBE
      }
      verdicts.push({
        market: 'moneyline',
    ...
    // 1ª mitad (sin momios publicados: barras, tope MAYBE)
    {
      ...
      verdicts.push({
        ...
        prob: p != null ? +p.toFixed(3) : null,
        edge: null,
        verdict: barVerdict(p, 1.01, 0.575),
