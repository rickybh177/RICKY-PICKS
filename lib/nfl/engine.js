...
/* Simula el juego completo. expHome/expAway ya traen TODOS los
   ...
   noOT: pretemporada — sin tiempo extra, el empate se queda. */
function simulateGame({ expHome, expAway, seed, noOT }) {
  ...
  for (let s = 0; s < N_SIMS; s++) {
    ...
    let hp = 0, ap = 0, h1h = 0, h1a = 0, hTds = 0, aTds = 0;
    // la 1ª mitad anota ~48% de los puntos (la 2ª más, por urgencia
    // y manejo de reloj); ceil(drives/2) sesgaba TODOS los 1H al over
    const half1 = Math.round(drives * 0.48);
    for (let d = 0; d < drives; d++) {
      const inH1 = d < half1;
      let r = rng();
      if (r < H.tdP) { hp += 7; hTds++; if (inH1) h1h += 7; }
      else if (r < H.tdP + H.fgP) { hp += 3; if (inH1) h1h += 3; }
      r = rng();
      if (r < A.tdP) { ap += 7; aTds++; if (inH1) h1a += 7; }
      else if (r < A.tdP + A.fgP) { ap += 3; if (inH1) h1a += 3; }
    }
    // TDs valen 6.95 en promedio: ~5% de los extras se fallan (sobre TDs reales)
    if (rng() < hTds * 0.05) hp -= 1;
    if (rng() < aTds * 0.05) ap -= 1;

    if (hp === ap) {
      // tiempo extra (en pretemporada no hay: el empate se queda)
      const r = rng();
      if (noOT || r < OT_TIE_P) { agg.ties++; }
...
function marketsFromSims(agg, lines) {
  ...
  return {
    ...
    /* distribución del total (14..70, colas agrupadas): permite que
       el frontend recalcule O/U contra CUALQUIER línea que el
       usuario meta de su casa de apuestas (panel de momios manuales) */
    total_dist: (() => {
      const counts = new Map();
      for (const t of agg.samples.totals) counts.set(t, (counts.get(t) || 0) + 1);
      const lo = 14, hi = 70, buckets = [];
      for (let t = lo; t <= hi; t++) {
        let c = counts.get(t) || 0;
        if (t === lo) { for (const [k, v] of counts) if (k < lo) c += v; }
        if (t === hi) { for (const [k, v] of counts) if (k > hi) c += v; }
        buckets.push(+(c / n).toFixed(4));
      }
      return { from: lo, p: buckets };
    })(),
  };
}
