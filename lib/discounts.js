...
const DISCOUNTS = {
  ...
  'AMIGOKIT':   { plan: 'ucl_temporada', price: 499, usd: 27.99 },
  ...
  'DAYOG':      { plan: 'europa_temporada', price: 2399, usd: 129.99 },
};
...
/* Precio final en MXN. ÚNICO lugar donde se calcula: stripe-create,
   ...
   no puedan discrepar. */
function priceWith(discount, listPrice, moneda) {
  if (!discount) return listPrice;
  /* Un código de PRECIO FIJO tiene un importe por moneda. Si le falta el
     de la moneda que se está cobrando se devuelve el de lista: jamás el
     número de la otra moneda (un $499 de pesos cobrado en dólares serían
     17 veces de más). Los porcentuales valen igual en cualquier moneda. */
  if (discount.price != null || discount.usd != null) {
    const fijo = moneda === 'MXN' ? discount.price : discount.usd;
    if (fijo == null) return listPrice;
    return Math.min(fijo, listPrice);
  }
  return Math.round(listPrice * (1 - discount.pct / 100) * 100) / 100;
}
