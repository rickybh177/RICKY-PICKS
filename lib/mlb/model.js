...
const { FALLBACK_LEAGUE, precomputeSide, simulateGame, marketsFromAgg } = require('./engine');
...
async function buildDay(dateISO, opts = {}) {
  ...
  // datos en paralelo: personas + bullpen y bateo de cada equipo
  const teamList = [...teamIds];
  const [people, bullpens, teamBats] = await Promise.all([
  ...
  for (const g of games) {
    try {
      out.push(buildGame(g, { people, bullpenByTeam, teamBatByTeam, lg, nSims, dateISO }));
    } catch (e) {
      out.push({
        gamePk: g.gamePk,
...
function buildGame(g, ctx) {
  ...
  return {
    ...
    verdicts: buildVerdicts(markets, teamMeta(homeId, homeT.team.name), teamMeta(awayId, awayT.team.name)),
...
function buildVerdicts(m, home, away) {
  ...
  const totalProb = overBest ? m.total.over : m.total.under;
  ...
  return [
    {
      ...
      verdict: verdictFor(favML, PICK_BAR.ML),
      prob: favML,
    },
    {
      market: 'TOTAL',
      label: `${overBest ? 'Más' : 'Menos'} de ${m.total.line} carreras`,
      verdict: verdictFor(totalProb, PICK_BAR.TOTAL),
      prob: totalProb,
    },
    {
      market: 'F5',
