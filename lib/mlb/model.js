...
const { FALLBACK_LEAGUE, precomputeSide, simulateGame, marketsFromAgg, probOverHist, probCoverHist } = require('./engine');
...
function buildGame(g, ctx) {
  ...
  if (mkt && (mkt.total != null || mkt.ml_home_prob != null)) {
    ...
    // handicap real del juego: P(el local cubre) en la línea que las casas ofrecen
    const pHomeCover = mkt.rl_point != null ? probCoverHist(agg.marginHist, agg.n, mkt.rl_point) : null;
    marketView = {
      ...
      rl_point: mkt.rl_point,
      rl_home: mkt.rl_home, rl_away: mkt.rl_away,
      rl_home_prob: mkt.rl_home_prob, rl_away_prob: mkt.rl_away_prob,
      rl_home_cover: pHomeCover,
    };
    if (pOverMkt != null) {
    ...
    if (pHomeCover != null) {
      markets.run_line_market = {
        point: mkt.rl_point, // handicap del LOCAL (ej. -1.5)
        home_cover: Math.round(pHomeCover * 10000) / 10000,
        away_cover: Math.round((1 - pHomeCover) * 10000) / 10000,
      };
    }
  }
...
function buildVerdicts(m, home, away, mv) {
  ...
  /* HANDICAP (run line): SIEMPRE a la línea real que ofrecen las
     casas para ESTE juego (ej. -1.5 / +1.5), nunca una inventada.
     Se muestra el lado donde el modelo ve más valor vs el precio. */
  let rlRow = null;
  if (mv && mv.rl_point != null && mv.rl_home_cover != null) {
    const fmtPt = p => (p > 0 ? `+${p}` : `${p}`);
    const sides = [
      { team: home, point: mv.rl_point, p: mv.rl_home_cover, mktP: mv.rl_home_prob, odds: mv.rl_home },
      { team: away, point: -mv.rl_point, p: 1 - mv.rl_home_cover, mktP: mv.rl_away_prob, odds: mv.rl_away },
    ];
    // lado con más edge (modelo - mercado sin vig); sin precio, el más probable
    const best = sides.sort((a, b) => {
      const ea = a.mktP != null ? a.p - a.mktP : a.p - 0.5;
      const eb = b.mktP != null ? b.p - b.mktP : b.p - 0.5;
      return eb - ea;
    })[0];
    let rlEdge = best.mktP != null ? Math.round((best.p - best.mktP) * 1000) / 1000 : null;
    let rlVerdict;
    if (rlEdge != null) {
      rlVerdict = (rlEdge >= 0.04 && best.p <= 0.80) ? 'bet' : rlEdge >= 0.015 ? 'maybe' : 'skip';
      if (rlEdge > 0.12 && rlVerdict === 'bet') rlVerdict = 'maybe'; // edge irreal = info que no tenemos
    } else {
      const bar = best.point < 0 ? PICK_BAR.RL_MINUS : PICK_BAR.RL_PLUS;
      rlVerdict = verdictFor(best.p, bar);
    }
    rlRow = {
      market: 'RL',
      label: `${best.team.name} ${fmtPt(best.point)}`,
      verdict: rlVerdict,
      prob: Math.round(best.p * 10000) / 10000,
      edge: rlEdge,
      line_txt: best.odds != null ? `mercado ${best.odds > 0 ? '+' + best.odds : best.odds}` : null,
    };
  } else {
    // sin mercado: ±1.5 estándar del modelo; el mejor lado normalizado
    const favM15 = homeFav ? m.run_line.home_minus_1_5 : m.run_line.away_minus_1_5;
    const dogP15 = homeFav ? m.run_line.away_plus_1_5 : m.run_line.home_plus_1_5;
    const dog = homeFav ? away : home;
    const sMinus = (favM15 - PICK_BAR.RL_MINUS) / (1 - PICK_BAR.RL_MINUS);
    const sPlus = (dogP15 - PICK_BAR.RL_PLUS) / (1 - PICK_BAR.RL_PLUS);
    rlRow = sMinus >= sPlus
      ? { market: 'RL', label: `${fav.name} -1.5`, verdict: verdictFor(favM15, PICK_BAR.RL_MINUS), prob: favM15 }
      : { market: 'RL', label: `${dog.name} +1.5`, verdict: verdictFor(dogP15, PICK_BAR.RL_PLUS), prob: dogP15 };
  }
...
function derivePicks(m) {
  ...
  const homeFav = m.moneyline.home >= m.moneyline.away;
  /* Run line: si hay línea REAL del mercado (run_line_market), los
     picks se evalúan en ESA línea; si no, en el ±1.5 estándar. */
  let rlCands;
  const rlm = m.run_line_market;
  if (rlm && rlm.point != null) {
    const fmtPt = p => (p > 0 ? `+${p}` : `${p}`);
    const minusSide = rlm.point < 0
      ? { sel: `${home} ${fmtPt(rlm.point)}`, prob: rlm.home_cover }
      : { sel: `${away} ${fmtPt(-rlm.point)}`, prob: rlm.away_cover };
    const plusSide = rlm.point < 0
      ? { sel: `${away} ${fmtPt(-rlm.point)}`, prob: rlm.away_cover }
      : { sel: `${home} ${fmtPt(rlm.point)}`, prob: rlm.home_cover };
    rlCands = [
      { market: 'RL', bar: PICK_BAR.RL_MINUS, ...minusSide },
      { market: 'RL', bar: PICK_BAR.RL_PLUS, ...plusSide },
    ];
  } else {
    const favM15 = homeFav ? m.run_line.home_minus_1_5 : m.run_line.away_minus_1_5;
    const dogP15 = homeFav ? m.run_line.away_plus_1_5 : m.run_line.home_plus_1_5;
    rlCands = [
      { market: 'RL', bar: PICK_BAR.RL_MINUS, sel: `${homeFav ? home : away} -1.5`, prob: favM15 },
      { market: 'RL', bar: PICK_BAR.RL_PLUS, sel: `${homeFav ? away : home} +1.5`, prob: dogP15 },
    ];
  }
  const cands = [
