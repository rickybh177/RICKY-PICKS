...
/* Calibración global de eventos positivos. Tuneada 6-ago-2026 con
   el backtest leak-free de 261 juegos (jul-14→ago-4, --sweep):
   con el valor viejo (1.026, de jul-7) el modelo esperaba 10.25
   carreras/juego vs 8.74 reales y sobre-predecía carrera en la 1ª.
   Con 0.97 + INN1_MULT 1.0: esperado 8.88 vs 8.74 real, NRFI
   53.4% vs 52.9% real, Brier ML igual. Re-tunear con --sweep. */
let CAL_EVENT = 0.97;
...
/* ============================================================
   ...
   ============================================================ */
/* Boost de 1ª entrada. En 1.0 desde 6-ago-2026: la simulación ya
   captura que la 1ª la abre el top del lineup (efecto mecánico), y
   el boost extra 1.06 duplicaba el efecto — el backtest mostraba
   demasiadas carreras tempranas (YRFI 54% modelo vs 47% real). */
let INN1_MULT = 1.0;

/* Re-tuneo de calibración (usado por el sweep del backtest y para
   fijar los valores de producción en un solo lugar). */
function setCalibration({ event, inn1 } = {}) {
  if (Number.isFinite(event)) CAL_EVENT = event;
  if (Number.isFinite(inn1)) INN1_MULT = inn1;
}
function getCalibration() { return { event: CAL_EVENT, inn1: INN1_MULT }; }
...
function simulateGame(gamePre, nSims, seed) {
  ...
  for (let s = 0; s < nSims; s++) {
    ...
    const sides = [
      { grid: gamePre.away.grid, order: 0, starterBF: 0, exitBF: sampleExitBF(gamePre.away.starterAvgBF, rng), runs: 0, f5: 0, inn1: 0, starterRuns: 0 },
      { grid: gamePre.home.grid, order: 0, starterBF: 0, exitBF: sampleExitBF(gamePre.home.starterAvgBF, rng), runs: 0, f5: 0, inn1: 0, starterRuns: 0 },
    ];
...
function halfInning(side, rng, inning, walkOffAway, homeRunsSoFar) {
  ...
  while (state.outs < 3) {
    // ¿sigue el abridor? Sale al llegar a su BF esperado, o ANTES si
    // lo están apaleando (gancho temprano del manager: 6+ carreras,
    // o 5 si ya va en la 2ª vuelta al lineup).
    let pitcherIdx;
    const shelled = side.starterRuns >= 6 ||
      (side.starterRuns >= 5 && side.starterBF >= 9);
    if (side.starterBF < side.exitBF && !shelled) {
    ...
    const paRuns = applyEvent(ev, state, rng);
    runs += paRuns;
    if (pitcherIdx !== 3) side.starterRuns += paRuns;
...
/* P(total > línea) sobre un histograma, con push descontado cuando
   la línea es entera (7.0, 8.0): prob = over / (1 - push). */
function probOverHist(hist, line, n) {
  if (line == null) return null;
  let over = 0, push = 0;
  const isInt = Math.floor(line) === line;
  for (let r = 0; r < hist.length; r++) {
    if (r > line) over += hist[r];
    else if (isInt && r === line) push += hist[r];
  }
  const eff = n - push;
  return eff > 0 ? over / eff : null;
}

/* P(el LOCAL cubre la línea `point` de handicap) sobre marginHist
   (margin = carreras local − visita). point negativo = local
   favorito (ej. −1.5: cubre si gana por 2+). Push descontado en
   líneas enteras (−2.0). La prob. de la visita es 1 − esto. */
function probCoverHist(marginHist, n, point) {
  if (point == null || !marginHist) return null;
  let cover = 0, push = 0;
  for (const [m, c] of marginHist) {
    if (m + point > 0) cover += c;
    else if (m + point === 0) push += c;
  }
  const eff = n - push;
  return eff > 0 ? cover / eff : null;
}
...
module.exports = {
  ...
  simulateGame, marketsFromAgg, probOverHist, probCoverHist,
