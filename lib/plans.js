...
const PLANS = {
  ...
  /* ---- FÚTBOL DE EUROPA (Premier League, LaLiga, Bundesliga) ----
     Tres modelos independientes, cada uno con su producto propio
     (`epl`, `laliga`, `bundesliga`), más los combos que los juntan.

     PRECIOS PROVISIONALES: son marcadores de posición para dejar el
     cableado completo (checkout, entitlements, rutas de regreso). Van
     alineados a la estrategia de pricing recomendada el 5-sep-2026
     (Europa = UN modelo a $349; Todo = $599; Temporada Europa =
     $1,199), pero el precio final lo decide el dueño antes de poner a
     la venta; mientras, TODOS llevan `upcoming: true` y el modelo corre
     en beta privada (api/euro-picks.js). Salir a la venta = quitar
     `upcoming` plan por plan (y reflejar el precio en checkout.html y
     meta-purchase.js, que lo espejan). La recomendación es NO vender
     las ligas sueltas (epl/laliga/bundesliga_mensual): dejarlas con
     `upcoming` para siempre o retirarlas.
  ...
  /* Las tres ligas juntas, mes a mes ("el más elegido" de Europa). */
  europa_mensual: {
    id: 'europa_mensual',
    title: 'Suscripción mensual — Europa (Premier + LaLiga + Bundesliga + Champions)',
    price: 349,       // MXN — PROVISIONAL: mismo precio que un modelo (recomendación 5-sep-2026: Europa se vende como UN modelo)
    currency: 'MXN',
    days: 30,
  ...
  /* TODO RICKY·PICKS: los seis modelos en una sola mensualidad. */
  todo_mensual: {
    id: 'todo_mensual',
    title: 'Suscripción mensual — Todo RICKY·PICKS (6 modelos)',
    price: 599,       // MXN — PROVISIONAL: "$100 por modelo" (recomendación 5-sep-2026; combo_mensual $549 queda como legado congelado)
    currency: 'MXN',
    days: 30,
  ...
  /* Temporada europea completa en pago único: agosto → mayo (275
     días cubren hasta la última jornada de las tres ligas, fin de
     mayo 2027). Ancla = 9 meses del mensual de Europa: 9 × $349. */
  europa_temporada: {
    id: 'europa_temporada',
    title: 'Temporada 2026-27 completa — Europa (Premier + LaLiga + Bundesliga + Champions)',
    price: 1199,      // MXN — PROVISIONAL: paridad con las demás temporadas (recomendación 5-sep-2026)
    anchor: 3141,     // valor real pagando mes a mes: 9 × $349 (ago→may)
    currency: 'MXN',
    days: 275,        // 1-ago → ~3-may; alcanza la última jornada de las tres ligas
    products: ['epl', 'laliga', 'bundesliga', 'ucl'],
    upcoming: true,   // todavía no se vende (beta privada)
    // PAGO ÚNICO: sin `recurring`. Nunca se cobra otra vez.
  },
  /* Acceso permanente a las tres ligas: solo por código/soporte
     (price 0 = jamás comprable), igual que los demás permanentes. */
  europa_permanente: {
    id: 'europa_permanente',
    title: 'Europa (Premier + LaLiga + Bundesliga + Champions) — acceso permanente',
    price: 0,
    currency: 'MXN',
    days: 36500,      // ~100 años
    products: ['epl', 'laliga', 'bundesliga', 'ucl'],
...
/* ¿Lo que el cliente YA tiene le gana a un plan que se otorgaría
   ...
   todo_mensual pisaba mlb_temporada y europa_temporada (pases
...
/* Pases completos de pago único: al otorgarlos, cualquier mensualidad
   ...
   europa_temporada entra por lo mismo: quien la compra deja de pagar
...
const FULL_PASS_PLANS = ['mlb_temporada', 'mx_apertura', 'nfl_temporada', 'combo_2026', 'europa_temporada'];
...
/* Champions (ucl, 8-sep-2026) entra al paquete Europa: nunca se vende sola. */
