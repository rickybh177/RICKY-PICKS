...
/* ============================================================
   ...
   ============================================================ */
const UPGRADE_UN_MODELO = { tres_mensual: 499, todo_mensual: 699 };
...
const UPGRADE_TRES = { todo_mensual: 898 };
...
function precioDe(planId, ents) {
  ...
  if (f && f.plan === planId) {
    opciones.push({ price: f.price, motivo: 'fundador', models: f.models, count: f.count });
  }
  const u = upgradeSuscriptor(ents);
  if (u && u.precios[planId] != null) {
    opciones.push({ price: u.precios[planId], motivo: u.tipo, from: u.from, model: u.model,
      /* lo que ya paga hoy: el mensaje del upgrade se cuenta por el
         incremento, no por el descuento. */
      pagaHoy: (PLANS[u.from] || {}).price || null });
  }
  if (!opciones.length) return null;

  const mejor = opciones.reduce((a, b) => (b.price < a.price ? b : a));
  if (!(mejor.price < def.price)) return null;             // no se "descuenta" hacia arriba
  return { ...mejor, plan: planId, lista: def.price, ahorro: def.price - mejor.price,
    /* sube: cuánto MÁS pagaría al mes respecto a su plan de hoy. */
    sube: mejor.pagaHoy != null ? mejor.price - mejor.pagaHoy : null };
}
...
module.exports = { PLANS, founderPriceFor, FOUNDER_PLAN, upgradeSuscriptor, precioDe, UPGRADE_UN_MODELO, UPGRADE_TRES, ALL_MODELS, isChoosePlan, chooseCount, validChoice, isSubscription, isUpcoming, planCoversProduct, entitlementGrants, entitlementExpiry, coverageBeats, productsAlreadyCovered, comboPermanentDiscount, monthlyUpgradeFor, FULL_PASS_PLANS, EURO_PRODUCTS };
