...
/* ============================================================
   PRECIOS EN USD (16-sep-2026)

   El sitio MUESTRA dólares. Quien paga con tarjeta (Stripe) se le
   cobra en USD; quien paga con Mercado Pago se le cobra en MXN,
   porque una cuenta mexicana de MP no procesa dólares.

   Los importes NO son una conversión automática: son puntos de precio
   redondos elegidos por el dueño ($19.99 / $34.99 / $49.99). La tasa
   implícita anda en 17.5-18.5 MXN/USD. Si el peso se mueve fuerte, se
   ajustan A MANO aquí — nunca se calculan al vuelo, porque un tipo de
   cambio vivo haría que el precio publicado cambiara solo.

   REGLA DURA: un plan sin `usd` NO se puede cobrar con tarjeta (ver
   montoDe). Es a propósito: más vale rechazar la compra que cobrar el
   número de PESOS en dólares — serían 17 veces de más.
   ============================================================ */
const USD = {
  /* mensuales */
  mlb_mensual: 19.99, mx_mensual: 19.99, nfl_mensual: 19.99,
  epl_mensual: 19.99, laliga_mensual: 19.99, bundesliga_mensual: 19.99, ucl_mensual: 19.99,
  tres_mensual: 34.99,
  todo_mensual: 49.99,
  /* temporadas de pago único */
  mlb_temporada: 49.99, mx_apertura: 49.99, nfl_temporada: 49.99,
  epl_temporada: 49.99, laliga_temporada: 49.99, bundesliga_temporada: 49.99, ucl_temporada: 49.99,
  /* legado: solo comprables con una oferta prometida o por código */
  combo_2026: 64.99,
  europa_temporada: 199.99,
  /* Mundial (torneo terminado, se dejan por si acaso) */
  mexico: 10.99, torneo: 16.99, final: 5.99,
};

/* Anclas "por separado": múltiplos del mensual, igual que en MXN
   (2443 = 7 × 349 → 7 × 19.99). */
const USD_ANCHOR = {
  todo_mensual: 139.93,        // 7 × 19.99
  mx_apertura: 79.96,          // 4 × 19.99
  nfl_temporada: 119.94,       // 6 × 19.99
  epl_temporada: 179.91, laliga_temporada: 179.91,
  bundesliga_temporada: 179.91, ucl_temporada: 179.91,   // 9 × 19.99
};

for (const [id, def] of Object.entries(PLANS)) {
  if (USD[id] != null) def.usd = USD[id];
  if (USD_ANCHOR[id] != null) def.anchor_usd = USD_ANCHOR[id];
}

/* Moneda que cobra cada pasarela. */
/* MONEDA (17-sep-2026). El sitio publica y cobra en PESOS por defecto:
   con dólares, a la mayoría de los clientes mexicanos su banco les
   rechazaba el cargo o les cobraba comisión por compra internacional.
   El dólar se queda como OPCIÓN explícita, y solo con tarjeta —
   Mercado Pago México no procesa dólares. */
const MONEDA_DEFAULT = 'MXN';
const MONEDAS = ['MXN', 'USD'];
/* Qué monedas acepta cada pasarela (la primera es su default). */
const MONEDA_PASARELA = { stripe: ['MXN', 'USD'], mercadopago: ['MXN'] };

/* Normaliza lo que llegue del navegador a una moneda que de verdad
   existe. Cualquier cosa rara cae al peso: cobrar de más por un
   parámetro mal escrito sería el peor default posible. */
function normalizaMoneda(m, pasarela) {
  const M = String(m == null ? '' : m).trim().toUpperCase();
  const permitidas = pasarela ? (MONEDA_PASARELA[pasarela] || [MONEDA_DEFAULT]) : MONEDAS;
  return permitidas.includes(M) ? M : permitidas[0];
}

/* Importe de un plan en una moneda. Devuelve null si ese plan no tiene
   precio en esa moneda — el llamador debe RECHAZAR la compra, nunca
   caer al número de la otra. */
function montoDe(planId, moneda) {
  const def = PLANS[planId];
  if (!def) return null;
  if (moneda === 'USD') return def.usd != null ? def.usd : null;
  return def.price > 0 ? def.price : null;
}
...
/* Precios legados en las DOS monedas. El sitio cobra en dólares con
   tarjeta y en pesos con Mercado Pago (ver MONEDA_PASARELA), así que
   cualquier monto que antes era "un número de pesos" necesita su par
   en USD: si el número de pesos se colara al cobro en dólares se
   cobrarían ~17 veces de más. */
const COMBO_PERM_PRICE = 799;      // MXN (combo_2026 $1,199 menos $400)
const COMBO_PERM_USD = 42.99;      // el mismo salto sobre los $64.99
...
function comboPermanentDiscount(ents) {
  ...
  return { price: COMBO_PERM_PRICE, usd: COMBO_PERM_USD, product: [...owned][0] };
}
...
const MONTHLY_UPGRADE_USD = 10.99; // su par en dólares
...
function monthlyUpgradeFor(ents) {
  ...
  return { target, price: MONTHLY_UPGRADE_PRICE, usd: MONTHLY_UPGRADE_USD, from };
}
...
const FOUNDER_PRICES = { varios: { MXN: 549, USD: 29.99 }, uno: { MXN: 699, USD: 38.99 } };
...
function founderPriceFor(ents) {
  ...
  return {
    plan: FOUNDER_PLAN,
    precios: previos.size >= 2 ? FOUNDER_PRICES.varios : FOUNDER_PRICES.uno,
    models: [...previos],
    count: previos.size,
  };
}
...
/* ============================================================
   UPGRADE PARA QUIEN YA TIENE UN MODELO (16-sep-2026).

   Regla del dueño: al cliente con UNA mensualidad viva se le abaratan
   los dos planes grandes, para que dar el salto cueste poco:
     Tres modelos a elegir   $599 → $499
     Todos los modelos       $899 → $699

   Es permanente mientras no cancele, igual que el precio de fundador:
   si al segundo mes le saltara a $899 se sentiría a engaño y cancelaría.

   Condiciones: la mensualidad tiene que estar VIGENTE (no sirve una
   vencida — eso ya no es "tener un modelo") y cubrir EXACTAMENTE UN
   modelo. Quien ya trae los tres o todos no entra: no es un upgrade.
   ============================================================ */
const UPGRADE_UN_MODELO = { tres_mensual: { MXN: 499, USD: 27.99 }, todo_mensual: { MXN: 699, USD: 38.99 } };

/* Quien ya trae el plan de TRES paga $898 por subir a todos: los $599
   que ya paga más $299 (decisión del dueño, 16-sep). Es casi el precio
   de lista, así que aquí NO se vende como descuento — el mensaje va por
   el incremento ("por $299 más"), que es como él lo quiere contar. */
const UPGRADE_TRES = { todo_mensual: { MXN: 898, USD: 48.99 } };

function upgradeSuscriptor(ents) {
  let tres = null;
  for (const e of ents || []) {
    if (!e || !e.active) continue;
    const def = PLANS[e.plan];
    if (!def || !def.recurring) continue;                 // solo mensualidades
    if (def.choose) {                                      // "a elegir": los tres
      if (entitlementGrants(e, e.product)) tres = e.plan;
      continue;
    }
    const prods = Array.isArray(def.products) ? def.products
      : [e.product || String(e.plan).split('_')[0]];
    if (prods.length !== 1) continue;                      // exactamente UN modelo
    if (!entitlementGrants(e, prods[0])) continue;         // y que siga vigente
    return { tipo: 'un_modelo', from: e.plan, model: prods[0], precios: { ...UPGRADE_UN_MODELO } };
  }
  if (tres) return { tipo: 'tres_modelos', from: tres, precios: { ...UPGRADE_TRES } };
  return null;
}

/* Precio de ESTE plan para ESTE cliente: el mejor de los que le tocan
   (nunca el peor). Un fundador con dos modelos previos paga $549 por
   "todos"; ese precio le gana al $699 del upgrade, y así debe ser.

   ÚNICO lugar donde se decide: de aquí leen stripe-create,
   create-payment y my-access, para que lo que se pinta y lo que se
   cobra no puedan discrepar. Devuelve null si paga precio de lista. */
function precioDe(planId, ents, moneda) {
  const M = moneda === 'USD' ? 'USD' : MONEDA_DEFAULT;  // el sitio habla pesos por defecto
  const def = PLANS[planId];
  const lista = montoDe(planId, M);
  if (!def || lista == null) return null;
  const opciones = [];

  const f = founderPriceFor(ents);
  if (f && f.plan === planId && f.precios[M] != null) {
    opciones.push({ price: f.precios[M], motivo: 'fundador', models: f.models, count: f.count });
  }
  const u = upgradeSuscriptor(ents);
  if (u && u.precios[planId] && u.precios[planId][M] != null) {
    opciones.push({ price: u.precios[planId][M], motivo: u.tipo, from: u.from, model: u.model,
      /* lo que ya paga hoy: el mensaje del upgrade se cuenta por el
         incremento, no por el descuento. */
      pagaHoy: montoDe(u.from, M) });
  }
  if (!opciones.length) return null;

  const mejor = opciones.reduce((a, b) => (b.price < a.price ? b : a));
  if (!(mejor.price < lista)) return null;                // no se "descuenta" hacia arriba
  const r2 = n => Math.round(n * 100) / 100;
  return { ...mejor, plan: planId, moneda: M, lista, ahorro: r2(lista - mejor.price),
    /* sube: cuánto MÁS pagaría al mes respecto a su plan de hoy. */
    sube: mejor.pagaHoy != null ? r2(mejor.price - mejor.pagaHoy) : null };
}
...
module.exports = { PLANS, USD, montoDe, MONEDA_PASARELA, MONEDA_DEFAULT, MONEDAS, normalizaMoneda, founderPriceFor, FOUNDER_PLAN, upgradeSuscriptor, precioDe, UPGRADE_UN_MODELO, UPGRADE_TRES, ALL_MODELS, isChoosePlan, chooseCount, validChoice, isSubscription, isUpcoming, planCoversProduct, entitlementGrants, entitlementExpiry, coverageBeats, productsAlreadyCovered, comboPermanentDiscount, monthlyUpgradeFor, FULL_PASS_PLANS, EURO_PRODUCTS };
