...
function upgradeMensualUnModelo(ents) {
  for (const e of ents || []) {
    if (!e || !e.active) continue;
    ...
    const prods = Array.isArray(def.products) ? def.products
      : def.choose ? null                                  // "a elegir": ya trae varios
      : [e.product || String(e.plan).split('_')[0]];
    if (!prods || prods.length !== 1) continue;            // exactamente UN modelo
    if (!entitlementGrants(e, prods[0])) continue;         // y que siga vigente
    return { from: e.plan, model: prods[0], precios: { ...UPGRADE_UN_MODELO } };
  }
  return null;
}
...
function precioDe(planId, ents) {
  ...
  const u = upgradeMensualUnModelo(ents);
  if (u && u.precios[planId] != null) {
    opciones.push({ price: u.precios[planId], motivo: 'un_modelo', from: u.from, model: u.model });
  }
...
module.exports = { PLANS, founderPriceFor, FOUNDER_PLAN, upgradeMensualUnModelo, precioDe, UPGRADE_UN_MODELO, ALL_MODELS, isChoosePlan, chooseCount, validChoice, isSubscription, isUpcoming, planCoversProduct, entitlementGrants, entitlementExpiry, coverageBeats, productsAlreadyCovered, comboPermanentDiscount, monthlyUpgradeFor, FULL_PASS_PLANS, EURO_PRODUCTS };
