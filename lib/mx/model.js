...
const MKT_BLEND = 0.28;
function evVerdict(p, amOdds, pMarket, { bet = 0.08, maybe = 0.03, floor = 0.2 } = {}) {
...
async function buildBoard() {
  const [windowGames, results] = await Promise.all([getWindow(), getSeasonResults()]);
  ...
  for (const g of windowGames) {
    ...
    /* momios del mercado (DraftKings vía ESPN) */
    const market = g.market && (g.market.ml_home != null || g.market.total_line != null) ? g.market : null;
    ...
    // Doble oportunidad — el "seguro" del partido: 1X o X2 (el 12 vive en el detalle)
    {
      ...
      verdicts.push({
        ...
        edge: null, verdict: barVerdict(best.p, 0.84, 0.76),
        line_txt: best.txt + ' · justo ' + (probToAm(best.p) > 0 ? '+' : '') + probToAm(best.p),
      });
    }
    ...
    // Total de goles (línea del mercado si hay; 2.5 si no)
    {
      ...
      if (am != null) {
        ...
        const v = evVerdict(p, am, pMkt, { bet: 0.05, maybe: 0.015, floor: 0.3 });
    ...
    // Ambos anotan (sin momios en el scoreboard: barras)
    {
      ...
      verdicts.push({
        ...
        verdict: barVerdict(p, 0.60, 0.565),
        line_txt: 'BTTS · justo ' + (probToAm(p) > 0 ? '+' : '') + probToAm(p),
      });
    }
    ...
    out.push({
      ...
      market_source: market ? (g.market.provider || 'sportsbook') : null,
      verdicts,
      markets: {
        moneyline: {
