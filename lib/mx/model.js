...
async function buildBoard() {
  const [windowGames, results, consensusOdds] = await Promise.all([
    getWindow(), getSeasonResults(), getLigaMxOdds(), // única fuente de momios
  ]);
  ...
  for (const g of windowGames) {
    ...
    /* momios del mercado: SOLO consenso multi-casas (The Odds API,
       decisión del dueño 4-ago-2026). Sin match → sin momios: los
       veredictos se topan en MAYBE, no se inventa precio. */
