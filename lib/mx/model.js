...
function evVerdict(p, amOdds, pMarket, { bet = 0.03, maybe = 0.01, floor = 0.2 } = {}) {
...
async function buildBoard() {
  const [windowGames, results, consensusOdds, manualMap] = await Promise.all([
    getWindow(), getSeasonResults(), getLigaMxOdds(),
    getManualMap('mx'), // momios capturados (Playdoit) — mandan sobre todo
  ]);
  ...
  for (const g of windowGames) {
    ...
    /* momios del mercado: MANUALES (Playdoit, capturados por el
       dueño — los que de verdad se pueden tomar) > consenso
       multi-casas (The Odds API) > DraftKings vía ESPN. El spread
       (AH) solo viene de ESPN, así que se conserva de ahí. */
    ...
    const manual = lookupManual(manualMap, g.date, g.away.abbr, g.home.abbr);
    if (manual && manual.ml_home != null && manual.ml_away != null) {
      const prev = market;
      market = {
        provider: manual.source || 'Playdoit',
        ml_home: manual.ml_home,
        ml_draw: manual.ml_draw != null ? manual.ml_draw : (prev ? prev.ml_draw : null),
        ml_away: manual.ml_away,
        total_line: manual.total != null ? manual.total : (prev ? prev.total_line : null),
        total_over_odds: manual.total != null ? (manual.total_over != null ? manual.total_over : null)
                                              : (prev ? prev.total_over_odds : null),
        total_under_odds: manual.total != null ? (manual.total_under != null ? manual.total_under : null)
                                               : (prev ? prev.total_under_odds : null),
        spread_line: prev ? prev.spread_line : null,
        spread_home_odds: prev ? prev.spread_home_odds : null,
        spread_away_odds: prev ? prev.spread_away_odds : null,
      };
    }
    ...
    // Doble oportunidad — el "seguro" del partido: 1X o X2 (el 12 vive en el detalle)
    {
      ...
      verdicts.push({
        ...
        edge: null, verdict: barVerdict(best.p, 0.76, 0.70),
    ...
    // Total de goles (línea del mercado si hay; 2.5 si no)
    {
      ...
      if (am != null) {
        ...
        const v = evVerdict(p, am, pMkt, { bet: 0.025, maybe: 0.01, floor: 0.3 });
    ...
    // Ambos anotan (sin momios en el scoreboard: barras)
    {
      ...
      verdicts.push({
        ...
        verdict: barVerdict(p, 0.555, 0.535),
