...
/* `mercado` (opcional) fija QUÉ veredicto enseña la card, por texto
   dentro del label. Si ese mercado no existe en el partido, se cae solo
   al veredicto normal.

   TEMPORAL (9-sep-2026): partidos taquilleros fijados a mano para una
   campaña de publicidad del dueño. Se salta a propósito la regla de
   "no regalar el partidazo". QUITAR cuando termine la campaña. */
const OVERRIDES = [
  { home: 'Cruz Azul', away: 'América', mercado: 'Menos de 2.5 goles' },
];
...
/* ¿Este partido está fijado a mano? Devuelve el override (con su
   `mercado`) o null. Lo usa /api/mx-free para enseñar el mercado que
   pide el override en vez del que manda por probabilidad. */
