...
/* 4-ago-2026: site.api.espn.com regresa 403 a IPs de datacenter
   (Vercel) con UA no-navegador — tiró el pick gratis y el tablero.
   El espejo site.web.api.espn.com sirve el mismo API sin bloquear;
   queda como primario y el clásico de respaldo (mismo fix que
   lib/nfl/data.js). */
const ESPN = 'https://site.web.api.espn.com/apis/site/v2/sports/soccer/mex.1';
const ESPN_FALLBACK = 'https://site.api.espn.com/apis/site/v2/sports/soccer/mex.1';
...
async function fetchJson(url, ttlMs = 10 * 60 * 1000) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const tryUrl = i === 2 && url.startsWith(ESPN) ? url.replace(ESPN, ESPN_FALLBACK) : url;
    try {
...
/* ---- ESPN dejó de aceptar RANGOS de fechas (15-sep-2026) ----
   `?dates=20260715-20260731` empezó a contestar HTTP 400 "Failed to
   get events endpoint." y con eso se cayeron TODOS los modelos de
   fútbol a la vez (Liga MX, Premier, LaLiga, Bundesliga, Champions):
   la página mostraba "Algo salió mal".

   Lo que sí sigue aceptando: un día suelto (`?dates=20260915`) y un
   MES completo (`?dates=202609`). Se pide por MESES y se filtra al
   rango exacto — un mes por petición en vez de una por día, y como
   la llave del caché es el mes, ventanas distintas que caen en el
   mismo mes reusan la misma respuesta. */
function mesesDe(startISO, endISO) {
  const out = [];
  let cur = new Date(startISO.slice(0, 7) + '-01T00:00:00Z');
  const fin = new Date(endISO.slice(0, 7) + '-01T00:00:00Z');
  while (cur <= fin) {
    out.push(cur.toISOString().slice(0, 7).replace('-', ''));
...
async function getRange(startISO, endISO, ttlMs) {
  ...
  const meses = await Promise.all(mesesDe(startISO, endISO).map(m =>
    fetchJson(`${ESPN}/scoreboard?dates=${m}&limit=1000`, ttl)));
  const seen = new Set(), out = [];
  for (const j of meses) for (const ev of (j.events || [])) {
    const dia = String(ev.date || '').slice(0, 10);
    if (dia < startISO || dia > endISO) continue;   // el mes trae de más: se recorta
    if (seen.has(ev.id)) continue;
    seen.add(ev.id); out.push(parseEvent(ev));
  }
...
/* Ventana visible: ayer a +9 días (una jornada completa, o dos con
   doble fecha entre semana).

   Si la liga está EN PAUSA y esa ventana no trae ningún partido por
   jugar, se extiende hasta encontrar la PRÓXIMA jornada y se
   muestra completa. Sin esto el tablero se queda vacío justo en los
   breaks: pasó el 4-ago-2026 (pausa de selecciones, la J4 arrancaba
   el día 15 — a 11 días, fuera de la ventana de 9). */
const WINDOW_DAYS = 9;
...
async function getWindow() {
  ...
  const near = await getRange(from, to, 5 * 60 * 1000);
  if (near.some(g => !g.completed)) return near;

  // pausa: traer la próxima jornada completa
  const ahead = await getRange(to, day(now + LOOKAHEAD_DAYS * 86400000), 30 * 60 * 1000);
