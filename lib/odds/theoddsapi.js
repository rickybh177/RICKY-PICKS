...
/* TTL por deporte (minutos). Las ligas de Europa juegan una jornada
   por semana y sus líneas se mueven poco entre corridas: con 60 min
   las tres ligas juntas gastan ~4.3K créditos/mes en el peor caso
   (24/7), en vez de ~8.6K con los 30 min generales. Los deportes que
   no aparecen aquí conservan TTL_MIN tal cual. */
const SPORT_TTL_MIN = {
  soccer_epl: 60,
  soccer_spain_la_liga: 60,
  soccer_germany_bundesliga: 60,
  soccer_uefa_champs_league: 60,
  soccer_italy_serie_a: 60,
  soccer_france_ligue_one: 60,
};

async function fetchSport(sportKey, markets = 'h2h,totals') {
  if (!KEY) return null;
  const cacheKey = `${sportKey}|${markets}`;
  const baseTtl = SPORT_TTL_MIN[sportKey] || TTL_MIN;

  // Nivel 1: memoria del lambda (gratis, instantáneo)
  const hit = _cache.get(cacheKey);
  const ttlOf = entry => (entry && entry.remaining != null && entry.remaining < CREDIT_RESERVE ? baseTtl * 8 : baseTtl) * 60e3;
...
function consensus(ev, threeWay) {
  ...
  for (const bk of (ev.bookmakers || [])) {
    ...
    for (const mk of (bk.markets || [])) {
      ...
      if (mk.key === 'h2h') {
        ...
        if (ov && ov.point != null) { totLines.push(Number(ov.point)); tOv.push({ point: Number(ov.point), price: ov.price }); }
        if (un && un.point != null) tUn.push({ point: Number(un.point), price: un.price });
  ...
  /* Línea de totales de consenso = la que MÁS casas ofrecen (moda del
     punto; en empate, la más cercana a la mediana) y precios over/
     under SOLO entre casas en ESA línea — igual que el handicap. En
     fútbol las casas se reparten entre 2.5 y 3.5 en el mismo partido
     (5-sep-2026: 12/22 eventos de Premier, 15/17 de LaLiga, 11/11 de
     Bundesliga con líneas mezcladas): la mediana de precios de TODAS
     contaminaba el over 3.5 con precios de over 2.5 y el EV del
     veredicto de totales se calculaba contra un precio que no existe. */
  let totPoint = null, totOver = null, totUnder = null;
  if (totLines.length) {
    const cnt = new Map();
    for (const p of totLines) cnt.set(p, (cnt.get(p) || 0) + 1);
    const med = median(totLines);
    totPoint = [...cnt.entries()]
      .sort((x, y) => y[1] - x[1] || Math.abs(x[0] - med) - Math.abs(y[0] - med) || x[0] - y[0])[0][0];
    totOver = medAm(tOv.filter(t => t.point === totPoint).map(t => t.price));
    totUnder = medAm(tUn.filter(t => t.point === totPoint).map(t => t.price));
  }
  ...
  return {
    ...
    total_line: totPoint,
    total_over_odds: totOver,
    total_under_odds: totUnder,
...
/* ============================================================
   FÚTBOL GENÉRICO (Europa: Premier, LaLiga, Bundesliga…).
   Mismo consenso 3 vías que Liga MX, pero la identidad del equipo
   la resuelve el caller: `nameToId` traduce el nombre que publica
   The Odds API ("Brighton and Hove Albion") al id de equipo de ESPN
   ("331"). Acepta una función (nombre → id | null) o un objeto
   { nombre: id }. Vive en lib/euro/oddsnames.js por liga.
   Regresa lista o null (sin key / API caído). Solo pre-partido.
   ============================================================ */
async function getSoccerOdds(sportKey, nameToId) {
  const events = await fetchSport(sportKey);
  if (!events) return null;
  const resolve = typeof nameToId === 'function'
    ? nameToId
    : (name => (nameToId && nameToId[name] != null ? String(nameToId[name]) : null));
  return events.filter(isPreGame)
    .map(ev => ({ ...consensus(ev, true), home: resolve(ev.home_team), away: resolve(ev.away_team) }))
    .filter(e => e.home && e.away);
}

/* Busca el consenso de un partido por id de ESPN (local, visita) y
   fecha ±36 h — mismo criterio que findMxOdds. */
function findSoccerOdds(list, homeId, awayId, dateISO) {
  if (!list) return null;
  const h = String(homeId), a = String(awayId);
  const t = new Date(dateISO).getTime();
  return list
    .filter(e => e.home === h && e.away === a &&
      Math.abs(new Date(e.commence).getTime() - t) < 36 * 3600e3)
    .sort((x, y) => Math.abs(new Date(x.commence) - t) - Math.abs(new Date(y.commence) - t))[0] || null;
}
...
module.exports = {
  getLigaMxOdds, findMxOdds, getMlbOdds, getSoccerOdds, findSoccerOdds,
  hasKey, amToProb, probToAm, median, kvGet, kvPut, SPORT_TTL_MIN,
};
