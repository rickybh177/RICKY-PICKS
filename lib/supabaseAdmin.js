...
// Todos los productos que cubre un plan.
// El combo cubre MLB + Liga MX; el Círculo Fundador incluye además NFL
// (su promesa es "todos los modelos").
...
async function getAllUsersWithPlans() {
  ...
  const entMap = {};
  (ents || []).forEach(e => {
    const product = e.product || productForPlan(e.plan);
  ...
  return (users || []).map(u => {
    ...
    return {
      ...
      plans: list,
      products: byProduct, // { mundial?: {plan,active,updated_at}, mlb?: {...}, mx?: {...} }
    };
  });
}
