...
// Los combos (combo_*, europa_*, todo_*) otorgan VARIAS filas con el
...
function productForPlan(plan) {
  ...
  if (plan && plan.startsWith('europa_')) return 'europa'; // agrupador: las 3 ligas
...
// combo_total y circulo = 3, europa_* = 3 ligas, todo_* = 6 modelos);
...
function productsForPlan(plan) {
  ...
  if (plan && plan.startsWith('europa_')) return EURO_PRODUCTS.slice();
...
// lib/plans.js). Antes un todo_mensual / europa_mensual / combo_mensual
// reemplazaba mlb_temporada o europa_temporada por 30 días de mensual
