...
function productForPlan(plan) {
  ...
  if (plan && plan.startsWith('tres_')) return 'tres';     // agrupador: 3 modelos a elegir (las filas traen el producto real)
  if (plan && plan.startsWith('todo_')) return 'todo';     // agrupador: todos los modelos
...
function productsForPlan(plan) {
  ...
  /* "a elegir" (tres_mensual): los productos no son del plan sino del
     usuario — sus filas de entitlements o su elección guardada
     (lib/choices.js). Aquí, vacío; ver rowProducts / chosenProducts. */
  if (def && def.choose) return [];
...
// Productos que cubre UNA fila: la columna `product` cuando existe
// (una fila por producto desde la migración de julio; la ÚNICA forma de
// saber qué modelos eligió un tres_mensual), y el plan como respaldo
// para filas viejas sin producto.
function rowProducts(e) {
  return e && e.product ? [e.product] : productsForPlan(e && e.plan);
}

// Modelos elegidos de un plan "a elegir" para un usuario: primero sus
// filas activas con ese plan (la verdad desde el primer cobro), si no,
// la elección guardada en el checkout (lib/choices.js). null si nada.
async function chosenProducts(admin, userId, plan) {
  const { data } = await admin
    .from('entitlements')
    .select('plan, product, active')
    .eq('user_id', userId)
    .eq('plan', plan)
    .eq('active', true);
  const fromRows = (data || []).map(r => r.product).filter(Boolean);
  if (fromRows.length) return [...new Set(fromRows)].sort();
  const { loadChoice } = require('./choices');
  return loadChoice(userId, plan);
}
...
async function getEntitlement(userId, userEmail, product) {
  ...
  if (userEmail && ADMIN_EMAILS.includes(userEmail)) {
    /* `updated_at: ahora` NO es adorno: la vigencia se calcula como
       updated_at + days, así que sin él estas filas vencían 30 días
       después de la ÉPOCA — el 30 de enero de 1970. Mientras el acceso
       se miraba con la bandera `active` no se notaba; en cuanto empezó
       a compararse contra la vigencia, el admin se quedó sin sus
       propios modelos. */
    const ahora = new Date().toISOString();
    if (product === 'mlb') return { plan: 'mlb_fundador', active: true, product: 'mlb', updated_at: ahora };
    if (product === 'mx') return { plan: 'mx_fundador', active: true, product: 'mx', updated_at: ahora };
    if (product === 'nfl') return { plan: 'nfl_fundador', active: true, product: 'nfl', updated_at: ahora };
    /* Europa: el mensual de cada liga (epl_mensual, laliga_mensual,
       bundesliga_mensual) — así el admin ve las tres en beta privada. */
    if (EURO_PRODUCTS.includes(product)) return { plan: product + '_mensual', active: true, product, updated_at: ahora };
    return { plan: 'torneo', active: true, product: 'mundial', updated_at: ahora };
  }
  const admin = getAdmin();
  /* granted_at = alta ORIGINAL del acceso. No se mueve en las
     renovaciones (grantEntitlement solo escribe updated_at), así que es
     lo único fiable para saber desde cuándo alguien es cliente — de ahí
     depende el precio de fundador. */
  const { data, error } = await admin
    .from('entitlements')
    .select('plan, product, active, granted_at, updated_at')
    .eq('user_id', userId)
    .eq('active', true);
  if (error || !data || !data.length) return null;
  if (!product) return data[0];
  const match = data.find(e => rowProducts(e).includes(product));
  return match || null;
}
...
async function getEntitlements(userId, userEmail) {
  if (!userId) return [];
  if (userEmail && ADMIN_EMAILS.includes(userEmail)) {
    /* updated_at: ahora → siempre vigente. Ver el porqué en
       getEntitlement: sin él, el admin vencía en 1970. */
    const ahora = new Date().toISOString();
    return [
      { plan: 'torneo', active: true, product: 'mundial', updated_at: ahora },
      { plan: 'mlb_fundador', active: true, product: 'mlb', updated_at: ahora },
      { plan: 'mx_fundador', active: true, product: 'mx', updated_at: ahora },
      { plan: 'nfl_fundador', active: true, product: 'nfl', updated_at: ahora },
      /* Europa: una fila por liga, mismo atajo que getEntitlement. */
      ...EURO_PRODUCTS.map(p => ({ plan: p + '_mensual', active: true, product: p, updated_at: ahora })),
    ];
  }
  const admin = getAdmin();
  /* granted_at = alta ORIGINAL del acceso. No se mueve en las
...
async function grantEntitlement(userId, plan, opts) {
  const admin = getAdmin();
  const now = new Date().toISOString();
  const def = PLANS[plan] || {};
  let products;
  if (def.choose) {
    /* "a elegir": la elección viene con la compra (opts.products, de la
       metadata de Stripe o del concepto de MP). En renovaciones
       (opts.preferRows) mandan sus filas — un cambio de modelo desde
       Mis modelos las mueve y la metadata de la pasarela se queda
       vieja —; si no hay filas, la elección guardada en el checkout.
       Sin nada: se falla fuerte para que el webhook reintente. Nunca
       se adivina. */
    const { validChoice } = require('./plans');
    const fromRows = (opts && opts.preferRows) ? await chosenProducts(admin, userId, plan) : null;
    products = fromRows || validChoice(plan, opts && opts.products) || await chosenProducts(admin, userId, plan);
    if (!products) throw new Error(`grantEntitlement: ${plan} sin modelos elegidos para ${userId}`);
  } else {
    products = productsForPlan(plan); // los combos escriben varias filas (una por producto)
  }
  const granted = [], kept = [];
  ...
  if (!attempt1Error) return { granted, kept, products };
  ...
  if (!old.error && old.data && old.data.active) {
    ...
    if (coverageBeats(old.data, plan)) {
      ...
      return { granted: [], kept: [{ product, plan: old.data.plan }], products };
    }
  }
  ...
  return { granted: [product], kept: [], products };
}
...
module.exports = { getAdmin, getUserFromToken, getEntitlement, getEntitlements, grantEntitlement, getAllUsersWithPlans, productForPlan, productsForPlan, rowProducts, chosenProducts };
