...
/* Overrides manuales, en orden de prioridad — misma semántica que
   ...
   Formas de fijar un partido:
     { league, home, away }  el matchup exacto (el orden no importa)
     { league, team }        el PRÓXIMO partido de ese equipo, juegue
                             de local o de visita
   `mercado` (opcional) fija qué veredicto enseña la card, por texto
   dentro del label; si ese mercado no existe, cae al normal.

   TEMPORAL (9-sep-2026): taquilleros fijados a mano para una campaña
   de publicidad del dueño. Se salta a propósito la regla de "no
   regalar el partidazo". QUITAR cuando termine la campaña. */
const OVERRIDES = [
  { league: 'laliga', home: 'Levante', away: 'Barcelona', mercado: 'Ambos equipos anotan' },
  { league: 'bundesliga', home: 'Elversberg', away: 'Bayern', mercado: 'Ambos equipos anotan' },
  { league: 'epl', home: 'Man United', away: 'Man City', mercado: 'Ambos equipos anotan' },
  /* Champions: el partido del Man United, con el under de goles.
     (Sustituye al del Madrid: su próximo juego es hasta el 14-oct y
     el tablero solo carga la jornada en curso, así que nunca salía.) */
  { league: 'ucl', team: 'Man United', mercado: 'Menos de 3.5 goles' },
];
...
/* ¿El partido es el que fija este override? Acepta las dos formas:
   por matchup (home+away, en cualquier orden) o por equipo suelto. */
...
/* ¿Este partido está fijado a mano? Devuelve el override (con su
   `mercado`) o null — lo usa /api/euro-free para la card. */
function overrideDe(leagueId, g) {
  if (!g || !g.home || !g.away) return null;
  return OVERRIDES.find(ov => (!ov.league || ov.league === leagueId) && coincide(g, ov)) || null;
}
...
module.exports = { featuredGame, featuredId, overrideDe, OVERRIDES };
